...
def convert(source, to, format=None, extra_args=(), encoding="utf-8"):
    ...
    """
    return _convert(
        _read_file, _process_file, source, to, format, extra_args, encoding=encoding
    )
...
def _convert(reader, processor, source, to, format=None, extra_args=(), encoding=None):
    ...
    if format not in from_formats:
        raise RuntimeError(
            "Invalid input format! Expected one of these: " + ", ".join(from_formats)
        )
...
def get_pandoc_formats():
    ...
    try:
        p = subprocess.Popen(
            ["pandoc", "-h"], stdin=subprocess.PIPE, stdout=subprocess.PIPE
        )
    except OSError:
